...
# ===========================================================================
# IRIS ASSIST (propose-then-confirm) -- Leave Management
# ===========================================================================
...
@router.post(
    ...
    summary="Ask IRIS what it would do for leave management, without executing anything",
)
async def preview_leave_iris_action(
...
@router.post(
    ...
    summary="Confirm and run an IRIS action for leave management",
)
async def execute_leave_iris_action(
    payload: IrisMessageRequest,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    entry = await AuditService.record(
        db, entity_type="leave_iris", entity_id=current_user.id,
...
@router.get(
    "/leaves/iris/activity",
    response_model=list[AuditLogResponse],
    summary="Recent IRIS activity on leave management for the current user",
)
async def get_leave_iris_activity(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    from app.services.audit import AuditService

    items, _total = await AuditService.list_for_entity(db, entity_type="leave_iris", entity_id=current_user.id, page=1, page_size=5)
    return items
