...
import { Box, Stack, Typography, IconButton, Button, Popover, Divider, Tooltip } from '@mui/material';
...
const WeekCalendarView: React.FC<WeekCalendarViewProps> = ({
	...
	const { cardSx, glowBubbleSx, mutedColor, isDark } = useBookingCardStyles('#4EA8FF');
					...
					{days.map((day) => {
						...
						return (
							<Box key={day.format('YYYY-MM-DD')} sx={{ textAlign: 'center', pb: 1, position: 'relative' }}>
								<Typography variant="caption" sx={{ color: mutedColor, fontWeight: 700, textTransform: 'uppercase', fontSize: '0.65rem' }}>
									{day.format('ddd')}
								</Typography>
								<Box
									sx={{
					...
					<Box sx={{ position: 'relative', height: gridHeight }}>
						{hours.map((h) => (
							<Typography
								key={h}
								variant="caption"
								sx={{ position: 'absolute', top: (h - startHour) * HOUR_HEIGHT - 7, right: 8, color: mutedColor, fontSize: '0.65rem' }}
							>
