			...
			try {
				...
				const donutData = Object.entries(group)
					.map(([name, count], index) => ({
						name: SOURCE_LABELS[name.toLowerCase()] || name,
						value: count,
				...
				if (total === 0) {
					const mockItems = [
						{ name: 'Website', value: 3, color: COLORS[0] },
						{ name: 'LinkedIn', value: 2, color: COLORS[1] },
						{ name: 'Referral', value: 1, color: COLORS[2] },
					];
					setData(mockItems);
					setTotalLeads(6);
				} else {
				...
				{loading ? (
					...
					<Box display="flex" flexDirection="column" alignItems="center" sx={{ flexGrow: 1 }}>
						...
						<Box sx={{
							...
							{data.map((d) => {
								const percentage = Math.round((d.value / totalLeads) * 100);
								return (
...
export default LeadCategoryDonutPanel;
