...
export const DepartmentBreakdownCard: React.FC<DepartmentBreakdownCardProps> = ({ distribution, total }) => {
	const theme = useTheme();
	const isDark = theme.palette.mode === 'dark';

	const rows = useMemo(
		() => Object.entries(distribution).sort((a, b) => b[1] - a[1]).slice(0, 5),
		[distribution]
	);
	...
	return (
		<Box sx={{
			...
			<Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 2.5 }}>
				...
				<Typography variant="subtitle2" fontWeight={800}>Department Breakdown</Typography>
			</Stack>
							...
							<Box key={dept}>
								<Stack direction="row" justifyContent="space-between" alignItems="baseline" sx={{ mb: 0.6 }}>
									<Stack direction="row" spacing={1} alignItems="center">
										...
										<Typography variant="body2" fontWeight={700} noWrap sx={{ maxWidth: 140 }}>{dept}</Typography>
									</Stack>
