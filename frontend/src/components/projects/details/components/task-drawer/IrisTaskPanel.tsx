	...
	return (
		...
		>
			<Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
				{/* Header */}
				<IrisPanelHeader subtitle={task.title} onClose={onClose} />
				...
				<Box sx={{ flex: 1, overflowY: 'auto', p: 2.25, display: 'flex', flexDirection: 'column', gap: 2 }}>
					...
					<Box sx={{ mt: 0.5 }}>
						...
						{preview ? (
							<Stack
								spacing={1.25}
								sx={{ p: 1.75, borderRadius: '12px', border: '1px solid', borderColor: alpha(brand, 0.35), bgcolor: alpha(brand, 0.05), boxShadow: theme.palette.mode === 'dark' ? '0 4px 16px -8px rgba(0,0,0,0.5)' : '0 4px 16px -8px rgba(0,0,0,0.12)' }}
							>
