...
export const TaskDrawerSidebar: React.FC<TaskDrawerSidebarProps> = ({
  ...
  const selectedAssignee =
    owners.find((o) => o.id === task.assignee_id) || null;
  const selectedStatus =
    statuses.find((s) => s.id === task.status_id) || statuses[0];
  const selectedPriority =
    PRIORITIES.find((p) => p.value === task.priority) || PRIORITIES[1];
  const currentType = task.custom_fields?.task_type || {
    name: "Task",
    color: "#FF9800",
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <IconButton
            size="small"
            onClick={openPopover("moreActions")}
              ...
              <Stack direction="row" alignItems="center" spacing={1}>
                ...
                >
                  {(selectedAssignee.full_name ||
                    selectedAssignee.email)[0]?.toUpperCase()}
                </Avatar>
                <Typography
                  variant="body2"
              ...
              sx={{
                ...
                bgcolor:
                  s.id === task.status_id
                    ...
                    : "transparent",
                color:
                  s.id === task.status_id ? "primary.main" : "text.primary",
                "&:hover": { bgcolor: hoverBg },
              }}
            >
              {dotIcon(s.color)}
              <Typography
                variant="body2"
                sx={{ fontWeight: s.id === task.status_id ? 700 : 600, ml: 1 }}
              >
                {s.name}
              </Typography>
            </Box>
      ...
      >
        <Stack
          sx={{ minWidth: 240, py: 0.5, maxHeight: 300, overflowY: "auto" }}
        >
          <Box
            onClick={() => {
              onUpdateField({ assignee_id: undefined });
              closePopover();
            }}
            sx={{
              display: "flex",
          ...
          {owners.map((o) => (
            <Box
              key={o.id}
              onClick={() => {
                onUpdateField({ assignee_id: o.id });
                closePopover();
              }}
              sx={{
                display: "flex",
        ...
        <Stack sx={{ minWidth: 160, py: 0.5 }}>
          {PRIORITIES.map((p) => (
            <Box
              key={p.value}
              onClick={() => {
                onUpdateField({ priority: p.value });
                closePopover();
              }}
              sx={{
                display: "flex",
      ...
      >
        <Box sx={{ p: 2, width: 260 }}>
          <DatePicker
            ...
            onChange={(v) => {
              onUpdateField({ start_date: v || undefined });
              closePopover();
            }}
            format="DD-MMM-YYYY"
          />
      ...
      >
        <Box sx={{ p: 2, width: 260 }}>
          <DatePicker
            ...
            onChange={(v) => {
              onUpdateField({ due_date: v || undefined });
              closePopover();
            }}
            format="DD-MMM-YYYY"
      ...
      >
        <Stack spacing={1.5} sx={{ p: 2, width: 220 }}>
            ...
            {[1, 2, 4, 8, 16, 24].map((h) => (
              <Grid size={4} key={h}>
                <Button
                  ...
                  fullWidth
                  onClick={() => {
                    onUpdateField({ estimated_hours: h });
                    closePopover();
                  }}
                  sx={{
                    borderRadius: "100px",
                    ...
                    py: 0.5,
                    borderColor:
                      task.estimated_hours === h ? "primary.main" : "divider",
                    bgcolor:
                      task.estimated_hours === h
                        ? alpha(theme.palette.primary.main, 0.08)
                        : "transparent",
          ...
          <TextField
            ...
            autoComplete="off"
            slotProps={{
              htmlInput: { min: 0, step: 0.5, autoComplete: "new-password" },
            }}
            autoFocus
            onKeyDown={(e) => {
      ...
      >
        <Stack sx={{ minWidth: 160, py: 0.5 }}>
          <Box
            onClick={() => {
              onUpdateField({ billing_type: "billable" });
              closePopover();
            }}
            sx={{
              px: 2.25,
          ...
          <Box
            onClick={() => {
              onUpdateField({ billing_type: "non_billable" });
              closePopover();
            }}
            sx={{
              px: 2.25,
      ...
      >
        <Box sx={{ p: 2, width: 320 }}>
          <TaskTagsInput
            value={task.tags || []}
            onChange={(newTags: ProjectTaskTag[]) =>
        ...
        <Stack sx={{ minWidth: 220, gap: 1.5 }}>
          ...
          <Stack spacing={0.5}>
            {taskTypes.map((t) => {
              const isSelected =
                (
                  task.custom_fields?.task_type?.name || "Task"
