...
export const ManagerAllocationTable: React.FC<ManagerAllocationTableProps> = ({
	...
	const columns: ColumnDefinition<TeamMember>[] = [
		{ id: 'full_name', label: 'Teammate' },
		{ id: 'role', label: 'Role' },
		{ id: 'reporting_manager_id', label: 'Current Reporting Manager' },
		{ id: 'actions', label: 'Reassign Manager' }
	];
					...
					<Stack direction="row" spacing={1} alignItems="center">
						<FormControl size="small" sx={{ minWidth: 200 }}>
							<Select
								...
								onChange={(e) => onManagerChange(user, (e.target.value as string | number) === '' ? '' : Number(e.target.value))}
								disabled={isUpdating}
								displayEmpty
								sx={{ borderRadius: 4 }}
							>
	...
	return (
		<Stack spacing={3}>
						...
						>
							<FormControl size="small" sx={{ minWidth: { xs: '100%', sm: 200 } }}>
								...
								<Select
									...
									onChange={(e) => onBulkManagerChange((e.target.value as string | number) === '' ? '' : Number(e.target.value))}
			...
			<DataTable<TeamMember>
				...
				numSelected={users.filter((u) => selectedIds.includes(u.public_id)).length}
